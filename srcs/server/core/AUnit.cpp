...
//            return true;
    ...
    {
      if ((getMax(getX(time), unit->getX(time))
	   < getMin(getX(time) + _hitBox.first,
		    unit->getX(time) + unit->getHitBox().first))
	  && (getMax(getY(time), unit->getY(time))
	      < getMin(getY(time) + _hitBox.second,
		       unit->getY(time) + unit->getHitBox().second)))
	return true;
      return false;
    }
